...
python positive_control_2drug/positive_control_2drug_analysis.py \
  --adata positive_controls/GSE206741_qc_mad_scrublet_log1p.givino_curcum.SE600M.h5ad \
  --start-cell "DMSO_DMSO" \
  --target-cell "Givinostat_Curcumin" \
  --cell-col "cell_type" \
  --embed-key X_state \
  --model-dir "$ST_RUN" \
  --checkpoint "$ST_CKPT" \
  --output-dir runs/PC_givino_curcum_no_search_NC \
  --random-pairs 100 \
  --2drug-pair "['crizotinib', 'Panobinostat']" \
  --batch 5 \
  --MOA-pairs "['Multi-TK inhibitor', 'HDAC inhibitor']" \
...
#------------------------------------------------------------------------------------------------------------------------------------------------------
#supplementary 
#2 counter-factual examples where panobinostat + crizotinib performed ok relative to positive controls 

python positive_control_2drug/positive_control_2drug_analysis.py \
  --adata positive_controls/GSE206741_qc_mad_scrublet_log1p.givino_tanesp.SE600M.h5ad \
  --start-cell "DMSO_DMSO" \
  --target-cell "Givinostat_Tanespimycin" \
  --cell-col "cell_type" \
  --embed-key X_state \
  --model-dir "$ST_RUN" \
  --checkpoint "$ST_CKPT" \
  --output-dir runs/PC_givino_tanesp_no_search_NC \
  --random-pairs 100 \
  --2drug-pair "['crizotinib', 'Panobinostat']" \
  --batch 5 \
  --MOA-pairs "['Multi-TK inhibitor', 'HDAC inhibitor']" \
