...
python cell_converter.py \
  ...
  --model-dir "$ST_RUN" \
  --checkpoint "$ST_CKPT" \
  --output-dir runs/PC_CPA_givino_curcum_PLS \
...
python make_positive_control_search_report.py \
  --run-dir runs/PC_CPA_givino_curcum_PLS \
  --drug-a "crizotinib" \
  --drug-b "Panobinostat"
...
python cell_converter.py \
  ...
  --model-dir "$ST_RUN" \
  --checkpoint "$ST_CKPT" \
  --output-dir runs/PC_CPA_givino_tanesp_PLS \
  --algorithm diverse_beam \
  --path-overlap-penalty 25 \
  --max-depth 2 \
  --beam-size 128 \
...
python make_positive_control_search_report.py \
  --run-dir runs/PC_CPA_givino_tanesp_PLS \
  --drug-a "crizotinib" \
  --drug-b "Panobinostat"
